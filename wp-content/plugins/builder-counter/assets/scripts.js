...
(function ($) {
	function builder_counter_init(e,el,type) {
                var items = $('.module.module-counter',el);
                if(el && el.hasClass('module-counter') && el.hasClass('module')){
                    items = items.add(el);
                }
                function builder_counter() {
                    items.each(function(){
                        var $this = $(this),
                            $countup = $this.find('.bc-timer'),
                            id = $countup.attr('id'),
                            from = $countup.data('from'),
                            to = $countup.data('to'),
                            decimal = $countup.data('decimals'),
                            suffix = $countup.data('suffix'),
                            prefix = $countup.data('prefix');

                        $this.waypoint(function(){
                                new countUp(id,from,to, decimal, 4, { useEasing : true, useGrouping : true, separator : $countup.data( 'grouping' ), decimal : '.', prefix : prefix, suffix: suffix }).start();
                ...
                if(items.length>0){
                    Themify.LoadAsync(themify_vars.url+'/js/waypoints.min.js', function(){
                            Themify.LoadAsync(themify_vars.url+'/js/jquery.easy-pie-chart.min.js', builder_counter, null, null, function(){
