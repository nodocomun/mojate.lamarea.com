...
library:{type:"audio"},multiple:!1}).on("select",function(){var b=h.state().get("selection").first().toJSON(),f=0<a("#auto_draft").length;e.find('input[name^="'+c.id+'[url]"]').val(b.url);var g=e.find('input[name^="'+c.id+'[title]"]');!f&&a.trim(g.val())||g.val(b.title);g=e.find("textarea");(f||a.trim(g.val()))&&g.val(b.caption);d.addClass("ptb_uploaded")}).open()})})})(jQuery);
