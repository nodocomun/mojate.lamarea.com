    ...
    <div class="themify_builder_tabs" id="themify_builder_tabs_<?php echo esc_attr($id); ?>">
        <ul class="clearfix">
            <?php foreach ($field['tabs'] as $key => $tab) : ?>
                <li <?php if ($first === true): ?><?php $first = false; ?> class="current"<?php endif; ?>><a href="#tb_<?php echo esc_attr($id . '_' . $key); ?>"> <?php echo esc_html($tab['label']); ?> </a></li>
            <?php endforeach; ?>
        </ul>

        <?php foreach ($field['tabs'] as $key => $tab) : ?>
            <div id="tb_<?php echo esc_attr($id . '_' . $key); ?>" class="themify_builder_tab">
                <?php
                if ($styling) {
...
function themify_render_styling_settings($fields) {
    foreach ($fields as $styling):
        ...
        echo!$is_seperator ? '<div class="' . join(' ', $field_class) . '">' : '';
        if (isset($styling['label'])) {
            echo '<div class="themify_builder_label">' . esc_html($styling['label']) . '</div>';
        }
        echo!$is_seperator ? '<div class="themify_builder_input">' : '';
    ...
    <?php foreach ($field['options'] as $option): ?>
        ...
        <?php if ($option['type'] === 'multi') : ?>
            <div class="themify_builder_field <?php echo isset($option['wrap_with_class']) ? esc_attr($option['wrap_with_class']) : ''; ?>">
                ...
                <div class="<?php echo esc_attr($option['id']), ' tb_multi_fields tb_fields_count_', esc_attr(count($option['options'])) ?>">
        ...
        <div class="themify_builder_field <?php echo isset($option['wrap_with_class']) ? esc_attr($option['wrap_with_class']) : ''; ?>">
            ...
            <div class="themify_builder_input"<?php echo ( 'wp_editor' === $option['type'] ) ? ' style="width:100%;"' : ''; ?>>
                    ...
                    <?php if (!empty($option['colorpicker'])) : ?>
                        <div class="minicolors_wrapper">
                            <div class="minicolors minicolors-theme-default">
                                <input type="text" name="<?php echo $option['id']; ?>" class="<?php echo isset($option['class']) ? esc_attr($option['class']) : ''; ?> minicolors-input tb_lb_option_child" data-input-id="<?php echo $option['id']; ?>" <?php
                            ...
                            <input type="text" class="<?php echo isset($option['class']) ? esc_attr($option['class']) : ''; ?> color_opacity" <?php echo themify_builder_get_binding_data($option); ?> />
                        </div>
                    <?php else : ?>
                        <input name="<?php echo $option['id']; ?>" class="<?php echo isset($option['class']) ? esc_attr($option['class']) : ''; ?> tb_lb_option_child" type="text" data-input-id="<?php echo $option['id']; ?>" <?php
                        echo themify_builder_get_binding_data($option);
                        echo themify_builder_get_control_binding_data($option);
                        ?> />
                ...
                <?php elseif ('image' === $option['type'] || 'video' === $option['type'] || 'audio' === $option['type']): ?>
                    <input data-input-id="<?php echo $option['id'] ?>" name="<?php echo $option['id'] ?>" placeholder="<?php if (isset($option['value'])) echo esc_attr($option['value']); ?>" class="<?php echo esc_attr($option['class']); ?> themify-builder-uploader-input tb_lb_option_child" type="text"<?php echo themify_builder_get_control_binding_data($option); ?> /><br />
                    <div class="small">
    ...
    function themify_builder_module_settings_field($module_options, $module_name) {
        ...
        foreach ($module_options as $field):
            ...
            <?php if ($field['type'] !== 'builder'): ?>
                ...
                <?php
                if ($field['type'] === 'multi') {
                    echo '<div class="' . esc_attr($id) . ' tb_multi_fields tb_fields_count_' . esc_attr(count($field['fields'])) . (!empty($field['pushed']) ? ' ' . $field['pushed'] : '' ) . '">';
                        ...
                        <?php if ('text' === $field['type']): ?>
                            ...
                            <?php else : ?>
                                ...
                                <?php if (isset($field['unit'])): ?>
                                    <div class="selectwrapper">
                                        <select id="<?php echo esc_attr($field['unit']['id']); ?>" class="tb_lb_option" <?php
                                        echo $data;
                        ...
                        <?php elseif ('radio' === $field['type']): ?>
                            ...
                            <div id="<?php echo $field['id']; ?>" class="tb_lb_option tb-radio-input-container<?php echo $option_js ? ' tb-option-radio-enable' : ''; ?>"<?php echo $control; ?>>
                                ...
                                <?php foreach ($field['options'] as $k => $v): ?>
                                    ...
                                    ?>
                                    <input <?php if (is_array($v) && !empty($v['disable'])): ?>disabled="disabled"<?php endif; ?> id="<?php echo $field['id'] . '_' . $k; ?>" name="<?php echo $field['id']; ?>" type="radio" value="<?php echo esc_attr($k); ?>" <?php echo " $default_checked $data_el"; ?> <?php echo themify_builder_get_binding_data($field); ?>/>
        ...
        switch ($styling['type']) {
            ...
            case 'image_and_gradient' :
                ...
                <div class="tb-image-gradient-field">
                    ...
                    if (!$is_premium) {
                        ?>
                        <div class="themify_builder_lite hide_opacity">
                            <?php
                        }
                        themify_builder_styling_field($styling);
                        if (!$is_premium) {
...
function themify_render_row_fields($fields) {
    foreach ($fields as $field) {
        ...
        if ($field['type'] !== 'separator') {
            echo '<div class="themify_builder_field ' . esc_attr($wrap_with_class) . '">';
        ...
        echo ( $field['type'] !== 'separator' ) ? '</div></div>' : ''; // themify_builder_input,themify_builder_field
    }
}
